"""
This module is a modification of the adjacency matrices creating class made by Pedro
...
if __name__ == "__main__":
    temp = Adj_Mats('first_last_ref.pdb')
]    temp.get_atom_dists()
    for i in [4, 5, 6, 7, 8, 10]:
        temp.get_atom_adj(t=i)
        temp.get_elec_adj()
        
        sns.set()
        plot = sns.heatmap(temp.adjacency_graphs[0])
        plot.figure.savefig("LK7_{}A_atom_hm_extended.png".format(i), dpi=200)
        plt.close()
        
        sns.set()
        plot = sns.heatmap(temp.adjacency_graphs[1])
        plot.figure.savefig("LK7_{}A_atom_hm_alpha.png".format(i), dpi=200)
        plt.close()
        
        sns.set()
        plot = sns.heatmap(temp.elec_adjacency_graphs[0])
        plot.figure.savefig("LK7_{}A_elec_hm_extended.png".format(i), dpi=200)
        plt.close()
        
        sns.set()
        plot = sns.heatmap(temp.elec_adjacency_graphs[1])
        plot.figure.savefig("LK7_{}A_elec_hm_alpha.png".format(i), dpi=200)
        plt.close()
...
#        temp.get_atom_adj(t=i)
#        temp.get_elec_adj()
#        temp.make_eigenvalues()
#        
#        fig = plt.figure(figsize=(12, 8))
#        plt.hist(temp.eigenvalues[0], bins=100)
#        plt.xlim(-_range_dict[i], _range_dict[i])
#        plt.title("Eigenvalue Distribution {}A Extended".format(i))
#        plt.show()
#        fig.savefig("LK7_{}A_extended_eigs.png".format(i), dpi=200)
#        plt.clf()
#        
#        fig = plt.figure(figsize=(12, 8))
#        plt.hist(temp.eigenvalues[1], bins=100)
#        plt.xlim(-_range_dict[i], _range_dict[i])
#        plt.title("Eigenvalue Distribution {}A Fully Alpha".format(i))
#        plt.show()
#        fig.savefig("LK7_{}A_alpha_eigs.png".format(i), dpi=200)
#        plt.clf()
